...
RSES_WITHOUT_INPUT = [] # ['T0_CH_CERN_Disk']
INT_SETTINGS = {'availability_read': True, 'availability_write': True, 'availability_delete': True}
INT_ATTRIBUTES = {'ddm_quota': 0, 'cms_type': 'int', 'reaper': False}
...
def rewrite_protocols(protocols, pfns, read_write=False, enforce_prefix=True):
    ...
    if read_write:
        ...
        new_domains = {'lan': {'delete': 0, 'read': 0, 'write': 0},
                       'wan': {'delete': 0, 'read': 1, 'third_party_copy': 0, 'write': 0}}
    ...
    for protocol in protocols:
        ...
        if enforce_prefix and protocol['scheme'] == scheme:
            ...
            if 'web_service_path' in protocol['extended_attributes']:
                web_path = protocol['extended_attributes']['web_service_path']
                new_protocol['extended_attributes'] = {}
                new_protocol['extended_attributes']['web_service_path'] = web_path
                prefix = prefix.replace(web_path, '')
                new_protocol['prefix'] = prefix
        if protocol['scheme'] in ['gsiftp']:
...
if __name__ == '__main__':
    ...
    for rse_name in set(RSES_TO_SET+RSES_WITHOUT_INPUT):
