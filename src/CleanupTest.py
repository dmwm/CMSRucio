...
for obj in sorted(rses):
    ...
    for dsn in client.list_datasets_per_rse(rse):
        blockName = dsn['name']
        container, block = dsn['name'].split('#')
        for rule in itertools.chain(client.list_did_rules(scope='cms', name=blockName), client.list_did_rules(scope='cms', name=container)):
            if rule['rse_expression'].endswith('_Test') and not rule['rse_expression'].endswith('_Tape_Test'):
                print('Deleting', rule['id'], blockName, rule['rse_expression'], rule['account'])
                try:
                    client.delete_replication_rule(rule_id=rule['id'])
                except:
                    print(' Problem deleting rule')
"""
