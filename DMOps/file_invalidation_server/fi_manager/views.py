...
class FileInvalidationRequestSerializer(serializers.Serializer):
    ...
    mode = serializers.ChoiceField(choices=['global','local','dbs-only','rucio-only'],allow_blank=False)
...
class FileQueryView(APIView):
    ...
    def get(self, request, request_id=None, *args, **kwargs):
        ...
        status_order = ['aborted', 'failed', 'in_progress', 'queued', 'waiting_approval', 'success']
        ordering = Case(
        ...
        context = {'grouped': grouped, 'data': page_obj.object_list, "page_obj":page_obj, "is_paginated": page_obj.has_other_pages(),'page_range': custom_range, 'total_objects':len(data)}    
        if self.request.accepted_renderer.format == 'html':
