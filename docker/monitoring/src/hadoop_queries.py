...
def get_df_rses(spark):
    ...
    df_rses = (
        spark.read.format("avro").load(hdfs_rses_path)
...
def get_df_subscriptions(spark):
    ...
    df_subscriptions = (
        spark.read.format("avro").load(hdfs_subs_path)
...
def get_df_accounts(spark):
    ...
    df_accounts = (
        spark.read.format("avro").load(hdfs_rucio_accounts)
        .filter(col('DELETED_AT').isNull())
...
def get_df_account_limits(spark):
    ...
    df_accounts = (
        spark.read.format("avro").load(hdfs_rucio_account_limits)
...
def get_df_locks(spark):
    ...
    locks = (
        spark.read.format('avro').load(locks_path) 
...
def get_df_replicas(spark):
    ...
    return (
        spark.read.format('avro').load(replicas_path)
        .withColumn('rse_id', lower(_hex(col('RSE_ID'))))
...
def get_df_lost_files_dids(spark,starting_date:datetime = None):
    ...
    if starting_date is not None:
        return (
            spark.read.format('avro').load(HDFS_RUCIO_DIDS).filter(col('AVAILABILITY')=='L').filter(col('DID_TYPE')=='F')
...
def get_df_contents(spark):
    ...
    df_containers = (
        df_contents.filter(col('DID_TYPE')=='C').filter(col('CHILD_TYPE')=='D').select(['NAME','CHILD_NAME'])
    ...
    return (
        df_containers.join(df_datasets,df_containers.DATASET == df_datasets.NAME,how='left')
...
def get_df_closed_dids(spark):
    ...
    HDFS_RUCIO_DIDS     = "/project/awg/cms/rucio/{}/dids/part*.avro".format(TODAY)
    return (
        spark.read.format('avro').load(HDFS_RUCIO_DIDS)
...
def get_df_file_dids(spark):
    ...
    return (
        spark.read.format('avro').load(dids_path)
