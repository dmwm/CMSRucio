...
py_input_args=(--es_host os-cms.cern.ch/os --es_secret_file /etc/secrets/es-secret --es_index test-new-rucio-rules-count-volume)
# log all to stdout
spark-submit "${spark_submit_args[@]}" "$script_dir/rucio_rules_count_volume.py" "${py_input_args[@]}" 2>&1